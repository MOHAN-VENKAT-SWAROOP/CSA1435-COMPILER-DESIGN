...
char production_rules_arr[10][10], f[10], first[10], ck;
int main(int argc, char** argv) {
    int jm = 0, km = 0, i, choice;
    char c, ch;
    count = 4;
    strcpy(production_rules_arr[0], "S=AaAb");
    strcpy(production_rules_arr[1], "S=BbBa");
    strcpy(production_rules_arr[2], "A=#");
        ...
        if (production_rules_arr[j][0] == c) {
            if (production_rules_arr[j][2] == '#') {
                if (production_rules_arr[q1][q2] == '\0') {
                    first[n++] = '#';
                } else if (production_rules_arr[q1][q2] != '\0' && (q1 != 0 || q2 != 0)) {
                    first_set_function(production_rules_arr[q1][q2], q1, (q2 + 1));
                } else {
                    first[n++] = '#';
                }
